  ...
  return (
    <>
      <nav className="glass sticky top-0 z-[100] px-6 md:px-12 py-3.5 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
        ...
        <div className="flex items-center gap-2 md:gap-2.5 overflow-x-auto no-scrollbar py-2">
          <Link href="/search" className="flex-shrink-0 flex items-center gap-1.5 bg-primary/5 hover:bg-primary text-primary hover:text-white px-3.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
            <Navigation className="w-3.5 h-3.5" />
            <span className="hidden md:inline">Book Parking</span>
            <span className="md:hidden">Book</span>
          </Link>
          <Link href="/owner/new" className="flex-shrink-0 flex items-center gap-1.5 bg-emerald-500/5 hover:bg-emerald-600 text-emerald-600 hover:text-white px-3.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
            <PlusCircle className="w-3.5 h-3.5" />
            <span className="hidden md:inline">List Parking</span>
            <span className="md:hidden">List</span>
          </Link>
          <Link href="/#intelligence" className="flex-shrink-0 flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-600 dark:text-slate-300 px-3.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
            <BarChart3 className="w-3.5 h-3.5" />
            <span className="hidden md:inline">City Overview</span>
            <span className="md:hidden">City</span>
          </Link>
          
          <div className="h-6 w-px bg-slate-200 dark:bg-slate-800 mx-1.5 flex-shrink-0" />
          
          <button 
            onClick={() => setShowRoleModal(true)}
            className="flex-shrink-0 w-9 h-9 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-950 rounded-full flex items-center justify-center hover:bg-primary transition-all active:scale-95 shadow-lg"
